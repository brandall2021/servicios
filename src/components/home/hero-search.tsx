...
function ListingPreview({ listing }: { listing: MarketplaceListingDTO }) {
  return (
    <Link href={listingHref(listing)} className="group block">
      <article className="overflow-hidden rounded-[28px] border border-white/12 bg-white shadow-[0_24px_70px_rgba(3,15,37,0.35)] transition-all duration-220 group-hover:-translate-y-1">
        <ListingMedia
        ...
        <div className="space-y-3 p-5">
          ...
          <div className="flex items-end justify-between gap-3">
            ...
            <span className="inline-flex items-center gap-1 text-sm font-semibold text-orange-700 transition-transform duration-200 group-hover:translate-x-0.5">
              Ver detalle
...
export function HeroSearch({ featuredListings }: HeroSearchProps) {
  ...
  return (
    <section className="relative overflow-hidden bg-[#081F3D] py-8 sm:py-12 lg:py-16 noise-overlay">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(255,138,0,0.12),transparent_30%),linear-gradient(135deg,#081F3D_0%,#0B2A55_45%,#06162E_100%)]" />
      <div className="absolute inset-0 dot-pattern opacity-20" />

      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        <div className="grid gap-10 lg:grid-cols-[1.4fr_1.1fr] lg:items-center">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-5 text-4xl font-semibold tracking-[-0.05em] text-white sm:text-5xl lg:text-6xl xl:text-7xl text-balance animate-fade-up animate-fade-up-delay-1">
            ...
            <div className="mt-8 animate-fade-up animate-fade-up-delay-3">
              <div className="inline-flex h-11 items-center gap-1 rounded-full border border-white/10 bg-white/95 p-1 shadow-[0_8px_32px_rgba(0,0,0,0.2)]">
                {TYPES.map((t) => (
                  <button
                    key={t.value}
                    type="button"
                    ...
                    className={`h-9 rounded-full px-5 text-sm font-semibold transition-all duration-200 ${
                      type === t.value
              ...
              <form onSubmit={handleSubmit} className="mt-3 flex h-[60px] items-center gap-2 rounded-2xl bg-white p-2 shadow-[0_12px_40px_rgba(0,0,0,0.22)]">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="h-full w-full rounded-xl pl-12 pr-4 text-base text-stone-900 placeholder:text-zinc-400 focus:outline-none"
                  />
                </div>
                <button
                  type="submit"
                  className="btn-glow inline-flex h-[46px] shrink-0 items-center justify-center gap-2 rounded-xl px-6 text-base font-semibold active:scale-[0.98]"
                >
                  Buscar
                  <ArrowRight className="h-4 w-4" />
                </button>
            ...
            <div className="mt-5 flex flex-wrap gap-3 text-sm text-white/75 animate-fade-up animate-fade-up-delay-4">
              <Link href="/register?role=PROVIDER" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/8 px-4 py-2 transition-all duration-200 hover:bg-white/12 hover:text-white">
                <Store className="h-4 w-4 text-orange-300" />
                Quiero ofrecer
              </Link>
              <Link href="#categorias" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/8 px-4 py-2 transition-all duration-200 hover:bg-white/12 hover:text-white">
                Explorar categorías
              </Link>
              <Link href="/buscar" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/8 px-4 py-2 transition-all duration-200 hover:bg-white/12 hover:text-white">
          ...
          <div className="relative">
            ...
            <div className="relative space-y-4">
              {primary ? (
                ...
                <div className="overflow-hidden rounded-[28px] border border-white/12 bg-white/95 p-6 text-stone-900 shadow-[0_24px_70px_rgba(3,15,37,0.35)]">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {secondary.map((listing) => (
                  <Link key={listing.id} href={listingHref(listing)} className="group block">
                    <article className="overflow-hidden rounded-[24px] border border-white/12 bg-white/95 shadow-[0_12px_36px_rgba(3,15,37,0.2)] transition-all duration-220 group-hover:-translate-y-1">
                      <ListingMedia
                        src={listing.media[0]?.archivo}
                        alt={listing.title}
                        type={listing.type}
                      ...
                      <div className="p-4">
                        ...
                        <div className="mt-3 flex items-center justify-between gap-3">
                          ...
                          <span className="text-xs font-semibold text-orange-700">Ver</span>
                        </div>
                      </div>
                    </article>
                  </Link>
        ...
        <div className="mt-10 flex flex-nowrap gap-3 overflow-x-auto pb-1">
          {CATEGORIAS.slice(0, 8).map((cat) => (
            <Link
              ...
              className="inline-flex shrink-0 items-center gap-2 rounded-full border border-white/12 bg-white/10 px-4 py-2 text-sm text-white/80 backdrop-blur-sm transition-all duration-200 hover:bg-white/16 hover:text-white"
            >
