    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <Link href={`/servicios/${request.servicio.id}`} className="text-sm text-emerald-600 hover:underline">
          ...
          <Card>
            <CardContent className="p-5">
              <h2 className="font-semibold text-stone-900 mb-2 flex items-center gap-2">
                <FileText className="h-4 w-4 text-emerald-600" /> Detalle de la solicitud
              </h2>
            ...
            <Card>
              <CardContent className="p-5">
                <h2 className="font-semibold text-stone-900 mb-3 flex items-center gap-2">
                  <DollarSign className="h-4 w-4 text-emerald-600" /> Cotizaciones
                </h2>
                <div className="space-y-3">
                  {request.cotizaciones.map((q) => (
                    <div key={q.id} className={`p-4 rounded-xl border ${q.id === bestQuote?.id ? "border-emerald-200 bg-emerald-50/40" : "border-stone-200/70 bg-white"}`}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-medium text-stone-500 uppercase tracking-wider">
                          Versión {q.version}
                          {q.id === bestQuote?.id && request.cotizaciones.length > 1 && (
                            <span className="ml-2 text-emerald-600 font-semibold">(última)</span>
                          )}
                        </span>
        ...
        <div className="space-y-4">
          ...
          {!isProvider && request.status === "COTIZADO" && bestQuote && (
            <Card className="border-emerald-200 bg-emerald-50/40">
              <CardContent className="p-5">
                <span className="text-xs font-semibold text-emerald-700 uppercase tracking-wider">Cotización actual</span>
